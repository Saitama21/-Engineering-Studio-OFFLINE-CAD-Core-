...
import {renderAssemblyProductionSheet,renderComponentProductionSheet} from './drawing/assembly-production-sheet-v120.js';

const APP_VERSION='1.2.0';
const BUILD_LABEL='VERIFIED GEOMETRY CORE';
...
function renderTree(){
  const r=state.rec,n=r.nativeAssembly;
  const rows=[['Файл',state.fileName],['Формат','SLDASM'],['Адаптер','Verified Geometry v1.2.0'],['Контейнер',n.container],['Streams',n.streamCount||0],['Позиций',n.componentCount],['Вхождений',n.occurrenceCount],['Tess-блоков',n.faceBlocks||0],['Исходных треуг.',n.sourceTriangles||0],['Сценовых треуг.',n.triangles||0],...(r.counts.displayTriangles&&r.counts.displayTriangles!==r.counts.triangles?[['3D LOD',r.counts.displayTriangles],['Передача','Stack-safe']]:[]),['Размещено',n.mappedOccurrences||0]];
...
function renderDimensions(){
  ...
  if(r.geometryAvailable!==false&&d.length){
    ...
    $('#dimensionsTable').innerHTML='<tr><td colspan="4">v1.2.0 строит адаптивный лист и маркирует аналитически подтверждённые размеры как VERIFIED. Нативный Parasolid B-Rep пока не декодируется.</td></tr>';
  }
}
