...
const worker=new Worker(`./import-worker.js?v=${APP_VERSION}`,{type:'module'});
...
function transportReviver(key,value){
  if(!value||typeof value!=='object'||!value.$rozType)return value;
  if(value.$rozType==='Map')return new Map(value.entries||[]);
  if(value.$rozType==='Set')return new Set(value.values||[]);
  if(value.$rozType==='BigInt')return BigInt(value.value);
