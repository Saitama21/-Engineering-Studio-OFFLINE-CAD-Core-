...
async function importFile(file){
  ...
  log(`Импорт ${fileName}: SLDASM Assembly Transform decoder · ${(fileSize/1024).toFixed(1)} KB.`);
...
function renderAll(){
  ...
  if(geo)$('#selectionInfo').textContent='FaceTessellations + assembly transforms · вращайте модель';renderTree();renderFeatures();renderDimensions();renderAssembly();updateDrawingModeAvailability();renderCurrentDrawing();$('#exportDrawingBtn').disabled=!r;
}
...
function renderTree(){
  const r=state.rec,n=r.nativeAssembly;
  const rows=[['Файл',state.fileName],['Формат','SLDASM'],['Адаптер','Assembly Transforms v0.7.1'],['Контейнер',n.container],['Streams',n.streamCount||0],['Позиций',n.componentCount],['Вхождений',n.occurrenceCount],['Tess-блоков',n.faceBlocks||0],['Исходных треуг.',n.sourceTriangles||0],['Сценовых треуг.',n.triangles||0],['Размещено',n.mappedOccurrences||0]];
...
function renderDimensions(){
  ...
  if(r.geometryAvailable!==false&&d.length){
    ...
    $('#dimensionsTable').innerHTML='<tr><td colspan="4">Для точных технологических размеров нужен B-Rep слой; v0.7.1 показывает габариты из встроенной тесселяции.</td></tr>';
  }
}
