...
const BUILD_LABEL='ANALYTIC SECTION & CURVE CORE';
...
function renderTree(){
  ...
  const rows=[['Файл',state.fileName],['Формат','SLDASM'],['Адаптер','Feature Recognition Core v2.5 + CAD Edge Graph'],['Контейнер',n.container],['Streams',n.streamCount||0],...(Number.isFinite(n.documentProperties?.mass)?[['Масса',`${n.documentProperties.mass.toFixed(2).replace('.',',')} кг`]]:[]),['Позиций',n.componentCount],['Вхождений',n.occurrenceCount],['Tess-блоков',n.faceBlocks||0],['Исходных треуг.',n.sourceTriangles||0],['Сценовых треуг.',n.triangles||0],...(r.counts.displayTriangles&&r.counts.displayTriangles!==r.counts.triangles?[['3D LOD',r.counts.displayTriangles],['Передача','Stack-safe']]:[]),...(r.brep?[['B-Rep V',r.brep.counts?.vertices||0],['B-Rep E',r.brep.counts?.edges||0],['B-Rep F',r.brep.counts?.faces||0],['B-Rep Shell',r.brep.counts?.shells||0],['Topology',`${Math.round((r.brep.coverage||0)*100)}% ${r.brep.topologyComplete?'FULL':'LOD'}`]]:[]),...(r.manufacturing?[['Feature holes',r.manufacturing.counts?.holes||0],['Фаски · кандидаты',r.manufacturing.counts?.chamfers||0],['Скругления · кандидаты',r.manufacturing.counts?.fillets||0],['Sheet Metal',r.manufacturing.counts?.sheetMetal||0],['Материал',r.manufacturing.materials?.documentMaterial||'не найден в SLDASM']]:[]),['Размещено',n.mappedOccurrences||0]];
...
function renderDimensions(){
  ...
  if(r.geometryAvailable!==false&&d.length){
    ...
    $('#dimensionsTable').innerHTML='<tr><td colspan="4">v2.5.0 сохраняет VERIFIED-геометрию, строит V/E/F/Shell B-Rep topology и сохраняет ручную доводку листа. Exact Parasolid surface/curve data пока не декодируется.</td></tr>';
  }
}
