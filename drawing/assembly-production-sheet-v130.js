...
function renderMesh(rec,s,box,{stroke='#111',width=.72,detail=false,fixedScale=null,hiddenRemoval=true}={}){const M=mapView(rec.bounds,s,box,detail?2:8,fixedScale),edges=engineeringLinework(rec,s.dir,{featureCos:detail?.992:.985,tangentCos:.9997}),vb=hiddenRemoval?buildOcclusionField(rec,s,{targetPixels:detail?220000:175000}):null;let d='',visibleSegments=0;for(const e of edges){const samples=detail?29:21;for(const seg of visibleEdgeSegments(e,s,vb,{samples})){const a=M.P(seg[0]),b=M.P(seg[1]);if(Math.hypot(a[0]-b[0],a[1]-b[1])<.38)continue;d+=`M${a[0].toFixed(1)} ${a[1].toFixed(1)}L${b[0].toFixed(1)} ${b[1].toFixed(1)}`;visibleSegments++;}}const stats=reconstructionStats(rec,s.dir);return{svg:`<path d="${d}" fill="none" stroke="${stroke}" stroke-width="${width}" stroke-linecap="round" stroke-linejoin="round" data-reconstruction="v2.1" data-raw-edges="${stats.rawEdges}" data-engineering-edges="${stats.engineeringEdges}" data-visible-segments="${visibleSegments}"/>`,map:M,hiddenRemoval:!!vb,reconstruction:stats}}
...
function renderGeneralAssemblySheet(svg,rec,{projectName='SLDASM',fileName='',theme='light',mode='assemblyDetailed'}={}){
  ...
  s+=`<g font-family="Arial" fill="#111"><text x="${large?105:95}" y="${H-42}" font-size="7" fill="#555">ROZFOOD ENGINEERING STUDIO · Drawing Intelligence v2.1.0 · GENERAL · ${cfg.label} · ${large?'A1':'A2'} · ${esc(fileName)}</text><text x="${W-145}" y="${bomBox.y-12}" font-size="9" text-anchor="end">VERIFIED TESS</text></g>`;
  svg.innerHTML=s;
}
...
export function renderAssemblyProductionSheet(svg,rec,{projectName='SLDASM',fileName='',theme='light',mode='assemblyDetailed'}={}){
  ...
  s+=`<g font-family="Arial" fill="#111"><text x="95" y="944" font-size="7" fill="#555">ROZFOOD ENGINEERING STUDIO · Drawing Intelligence v2.1.0 · ${esc(fileName)}</text><text x="1240" y="612" font-size="9" text-anchor="end">VERIFIED TESS</text></g>`;
  svg.innerHTML=s;
}
...
function renderGeneralComponentSheet(svg,part,{componentName='Деталь',fileName='',theme='light'}={}){
  ...
  s+=`<text x="95" y="944" font-family="Arial" font-size="7" fill="#555">ROZFOOD ENGINEERING STUDIO · Drawing Intelligence v2.1.0 · GENERAL PART · ${esc(fileName)}</text><text x="1240" y="815" font-family="Arial" font-size="9" text-anchor="end" fill="#111">VERIFIED TESS</text>`;
  svg.innerHTML=s;
}
...
export function renderComponentProductionSheet(svg,rec,{componentId=null,componentName='Деталь',fileName='',theme='light'}={}){
  ...
  s+=renderPartStamp(componentName,scale,{x:860,y:845,w:495,h:95});
  s+=`<text x="95" y="944" font-family="Arial" font-size="7" fill="#555">ROZFOOD ENGINEERING STUDIO · Drawing Intelligence v2.1.0 · ${esc(fileName)}</text><text x="1240" y="815" font-family="Arial" font-size="9" text-anchor="end" fill="#111">VERIFIED TESS</text>`;
  svg.innerHTML=s;
}
