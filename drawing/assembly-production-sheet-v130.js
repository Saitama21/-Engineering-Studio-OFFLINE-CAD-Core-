...
import {engineeringLinework,reconstructionStats} from './drawing-reconstruction-core.js';
import {buildBodyVisibilityField,visibleBodyAwareEdgeSegments,bodyVisibilityStats} from './body-visibility-core.js';
import {buildExactVisibilityBVH,visibleExactEdgeSegments,exactCurveVisibilityStats} from '../core/exact-curve-visibility-core.js';
import {buildAnalyticFaceHLR,visibleAnalyticFaceEdgeSegments,analyticFaceHLRStats} from '../core/analytic-face-hlr-core.js';
import {cleanProjectedSegments,segmentsPath} from './linework-cleanup-core.js';
import {resolveProjectedCadEdges} from './cad-edge-visibility-core.js';
...
import {planAssemblySections} from './assembly-section-intelligence.js';
import {reconstructSectionMaterial} from './section-material-core.js';
import {recognizeWeldedAssembly} from '../core/welded-assembly-geometry.js';
import {reconstructWeldSeams,renderWeldSeams,renderLocalWeldDetail} from './weld-seam-joint-core.js';
import {featureDrawingNotes,reconstructCadFeatureEntities} from '../core/cad-feature-entities.js';
import {sectionSolidRegionContours,reconstructSolidRegions} from '../core/solid-region-core.js';
import {reconstructExactSectionRegions} from '../core/exact-section-region-core.js';
import {reconstructExactSectionBoolean} from '../core/exact-section-boolean-core.js';
import {buildDrawingViewTopology} from './drawing-view-topology-core.js';
import {buildUnifiedDrawingView} from './unified-drawing-view-core.js';
import {composeDrumA2,composerDiagnostics} from './engineering-drawing-composer.js';
import {createAnnotationLayout} from './annotation-layout-core.js';
import {planDrumDimensions} from './semantic-dimension-planner.js';
import {buildFunctionalDimensionPlan} from './functional-dimension-core.js';
import {createProductionLeaderRouter} from './production-leader-core.js';
import {sectionContextRecord} from './section-context-core.js';
import {createDrawingFidelityGuard} from './drawing-fidelity-guard.js';
import {buildProductionDraftingGraph,renderProductionHelicoids,renderProductionAAOverlay,renderProductionDetailD,referenceGradeStats} from './reference-grade-production-core.js';
import {runProductionDrawingQA} from './production-drawing-qa-core.js';
import {buildProductionViewSynthesis} from './production-view-synthesizer.js';
...
function renderMesh(rec,s,box,{stroke='#111',width=.72,detail=false,fixedScale=null,hiddenRemoval=true,frameBounds=null}={}){
  ...
  const unifiedView=buildUnifiedDrawingView(rec,s.dir,{detail,minConfidence:detail?.78:.82,creaseDeg:detail?6:10,tangentDeg:detail?.45:.8,samplesPerFace:detail?9:7});
  const analyticBundle=unifiedView.analyticBundle;
  const analytic=analyticBundle.analytic;
  const topologyView=unifiedView.topology;
  const rawEdges=topologyView.topologyUsable?topologyView.edges:engineeringLinework(rec,s.dir,{featureCos:detail?.994:.988,tangentCos:.99975});
  // v5.0: resolve coincident/overlapping assembly CAD boundaries in view-space before HLR.
  // This prevents different bodies from feeding the same projected line into the occlusion pass.
  const projectedGraph=resolveProjectedCadEdges(rawEdges,s,rec,{angleDeg:detail?.06:.08});
  const edges=projectedGraph.edges;
  const exactVisibility=hiddenRemoval?buildExactVisibilityBVH(rec):null;
  const analyticFaceHLR=hiddenRemoval?buildAnalyticFaceHLR(rec):null;
  const vb=hiddenRemoval?buildBodyVisibilityField(rec,s,{targetPixels:detail?1150000:900000}):null;
  const solveRaster=(edge,samples,refine)=>visibleBodyAwareEdgeSegments(edge,s,vb,{samples,refine});
  const solveLegacyExact=(edge,samples,refine)=>exactVisibility?.root?visibleExactEdgeSegments(edge,s.dir,exactVisibility,{samples,refine}):solveRaster(edge,samples,refine);
  const solveFaceExact=(edge,samples,refine)=>analyticFaceHLR?visibleAnalyticFaceEdgeSegments(edge,s.dir,analyticFaceHLR,{samples,refine}):solveLegacyExact(edge,samples,refine);
  const contourSegs=[],featureSegs=[],analyticSegs=[];let visibleSegments=0,analyticSegments=0,suppressedAnalyticMesh=0,selfOccludedSamples=0,foreignOccludedSamples=0;
  for(const e of edges){
    if(edgeIsAnalyticSurface(e,analytic)){suppressedAnalyticMesh++;continue}
    const samples=detail?41:33,target=(e.kind==='SILHOUETTE'||e.kind==='BOUNDARY')?contourSegs:featureSegs;
    const vis=(e.kind==='SILHOUETTE'||e.kind==='BOUNDARY')?solveFaceExact(e,detail?11:9,6):solveRaster(e,detail?27:21,7);selfOccludedSamples+=vis.stats.selfOccluded;foreignOccludedSamples+=vis.stats.foreignOccluded;
    for(const seg of vis.segments){
      const a=M.P(seg[0]),b=M.P(seg[1]);if(Math.hypot(a[0]-b[0],a[1]-b[1])<.42)continue;target.push({a,b});visibleSegments++;
    }
  }
  for(const curve of analyticBundle.curves){
    const pts=curve.points||[];if(pts.length<2)continue;const target=curve.silhouette||curve.role==='surface-silhouette'?contourSegs:analyticSegs;
    for(let i=0;i<pts.length-1;i++){const edge={a:pts[i],b:pts[i+1],componentId:curve.componentId,componentIds:curve.componentIds,contributors:curve.contributors,faceKey:curve.faceKey,faceKeys:curve.faceKeys||curve.source?.faceKeys};const vis=(curve.silhouette||curve.role==='surface-silhouette')?solveFaceExact(edge,detail?7:5,6):solveRaster(edge,detail?15:11,6);selfOccludedSamples+=vis.stats.selfOccluded;foreignOccludedSamples+=vis.stats.foreignOccluded;for(const seg of vis.segments){const a=M.P(seg[0]),b=M.P(seg[1]);if(Math.hypot(a[0]-b[0],a[1]-b[1])<.25)continue;target.push({a,b});analyticSegments++}}
  }
  // v5.0: final screen cleanup remains conservative; most duplicate suppression now happens
  // upstream in the unified/view-aware CAD Edge Graph before HLR.
  // Clean each engineering line class independently. This removes residual CAD/mesh
  // fragments and joins only nearly-collinear touching pieces, without flattening real curves.
  const featureClean=cleanProjectedSegments(featureSegs,{quant:.16,minLength:.34,joinGap:.28,angleDeg:.28,lineTol:.13});
  const analyticClean=cleanProjectedSegments(analyticSegs,{quant:.13,minLength:.24,joinGap:.20,angleDeg:.12,lineTol:.08});
  const contourClean=cleanProjectedSegments(contourSegs,{quant:.16,minLength:.38,joinGap:.30,angleDeg:.30,lineTol:.14});
  const featureD=segmentsPath(featureClean.segments),analyticD=segmentsPath(analyticClean.segments),contourD=segmentsPath(contourClean.segments);
  const stats=reconstructionStats(rec,s.dir);
  // ISO/GOST-like visual hierarchy: visible outline strongest, analytic/form edges medium, details light.
  const contourWidth=Math.max(.48,width*.76),analyticWidth=Math.max(.36,width*.57),featureWidth=Math.max(.25,width*.40);
  const cleanup={feature:featureClean.stats,analytic:analyticClean.stats,contour:contourClean.stats};
  return{svg:`<g data-reconstruction="v9.0" data-drawing-view-topology="v9.0" data-view-dependent-surface-visibility="v9.0" data-unified-drawing-view-core="v9.0" data-true-surface-projection-core="v9.0" data-analytic-face-hlr-core="v9.0" data-parametric-helicoid-core="v9.0" data-parametric-helicoids="${analytic.counts.parametricHelicoids||0}" data-helicoid-silhouettes="${analytic.counts.helicoidSilhouetteCurves||0}" data-cad-edge-graph="v9.0" data-linework-cleanup="v9.0" data-body-aware-hlr="v9.0" data-exact-curve-visibility-core="v9.0" data-exact-curve-core="v6.0" data-cad-feature-core="v6.0" data-surface-continuity-core="v6.0" data-surface-type-core="v6.0" data-surface-primitive-core="v6.0" data-surface-intersection-core="v6.0" data-surface-trim-core="v6.0" data-surface-trimmed-curves="${analytic.counts.surfaceTrimmedCurves||0}" data-surface-trim-domains="${analytic.counts.surfaceTrimDomains||0}" data-surface-intersections="${analytic.counts.surfaceIntersectionCurves||0}" data-surface-types="${analytic.counts.surfaceTypes||0}" data-surface-cones="${analytic.counts.surfaceCones||0}" data-surface-tori="${analytic.counts.surfaceTori||0}" data-exact-silhouette-core="v6.0" data-exact-silhouettes="${analytic.counts.exactSilhouetteCurves||0}" data-exact-silhouette-cylinders="${analytic.counts.exactSilhouetteCylinders||0}" data-exact-silhouette-cones="${analytic.counts.exactSilhouetteCones||0}" data-exact-silhouette-tori="${analytic.counts.exactSilhouetteTori||0}" data-surface-primitives="${analytic.counts.surfacePrimitiveCurves||0}" data-surface-primitive-replaced="${analytic.counts.surfacePrimitiveReplacedEdges||0}" data-surface-suppressed="${analytic.counts.surfaceSuppressed||0}" data-tangent-edges-removed="${analytic.counts.tangentRemoved||0}" data-fillet-silhouettes="${analytic.counts.filletSilhouettes||0}" data-analytic-cylinders="${analytic.counts.cylinders}" data-exact-lines="${analytic.counts.exactLines||0}" data-exact-fillets="${analytic.counts.exactFillets||0}" data-exact-chamfers="${analytic.counts.exactChamfers||0}" data-feature-curves="${analytic.counts.featureCurves||0}" data-exact-arcs="${analytic.counts.exactArcs||0}" data-refined-boundaries="${analytic.counts.refinedBoundaries||0}" data-cad-boundaries="${analytic.counts.cadBoundaries||0}" data-helical-curves="${analytic.counts.helicalCurves||0}" data-helical-connectors="${analytic.counts.helicalConnectors||0}" data-suppressed-analytic-mesh="${suppressedAnalyticMesh}"><path d="${featureD}" fill="none" stroke="${stroke}" stroke-width="${featureWidth}" stroke-linecap="round" stroke-linejoin="round"/><path d="${analyticD}" fill="none" stroke="${stroke}" stroke-width="${analyticWidth}" stroke-linecap="round" stroke-linejoin="round"/><path d="${contourD}" fill="none" stroke="${stroke}" stroke-width="${contourWidth}" stroke-linecap="round" stroke-linejoin="round"/></g>`,map:M,hiddenRemoval:!!(exactVisibility?.root||vb),reconstruction:{...stats,analyticCylinders:analytic.counts.cylinders,exactLines:analytic.counts.exactLines||0,exactArcs:analytic.counts.exactArcs||0,exactFillets:analytic.counts.exactFillets||0,exactChamfers:analytic.counts.exactChamfers||0,featureCurves:analytic.counts.featureCurves||0,tangentCandidates:analytic.counts.tangentCandidates||0,tangentRemoved:analytic.counts.tangentRemoved||0,filletSilhouettes:analytic.counts.filletSilhouettes||0,g1Boundaries:analytic.counts.g1Boundaries||0,surfacePrimitiveCurves:analytic.counts.surfacePrimitiveCurves||0,surfaceIntersectionCurves:analytic.counts.surfaceIntersectionCurves||0,surfaceTrimmedCurves:analytic.counts.surfaceTrimmedCurves||0,surfaceTrimDomains:analytic.counts.surfaceTrimDomains||0,surfaceTrimTriangles:analytic.counts.surfaceTrimTriangles||0,surfacePrimitiveLines:analytic.counts.surfacePrimitiveLines||0,surfacePrimitiveArcs:analytic.counts.surfacePrimitiveArcs||0,surfacePrimitiveCircles:analytic.counts.surfacePrimitiveCircles||0,surfacePrimitiveReplacedEdges:analytic.counts.surfacePrimitiveReplacedEdges||0,surfaceTypes:analytic.counts.surfaceTypes||0,surfaceSuppressed:analytic.counts.surfaceSuppressed||0,surfacePlanes:analytic.counts.surfacePlanes||0,surfaceCylinders:analytic.counts.surfaceCylinders||0,surfaceCones:analytic.counts.surfaceCones||0,surfaceTori:analytic.counts.surfaceTori||0,surfaceSpheres:analytic.counts.surfaceSpheres||0,parametricHelicoids:analytic.counts.parametricHelicoids||0,helicoidSilhouetteCurves:analytic.counts.helicoidSilhouetteCurves||0,sphereSilhouetteCurves:analytic.counts.sphereSilhouetteCurves||0,exactSilhouetteCurves:analytic.counts.exactSilhouetteCurves||0,exactSilhouetteCylinders:analytic.counts.exactSilhouetteCylinders||0,exactSilhouetteCones:analytic.counts.exactSilhouetteCones||0,exactSilhouetteTori:analytic.counts.exactSilhouetteTori||0,surfaceHelical:analytic.counts.surfaceHelical||0,refinedBoundaries:analytic.counts.refinedBoundaries||0,cadBoundaries:analytic.counts.cadBoundaries||0,helicalCurves:analytic.counts.helicalCurves||0,helicalConnectors:analytic.counts.helicalConnectors||0,suppressedAnalyticMesh,analyticSegments,visibleSegments,viewTopology:topologyView.stats,unifiedDrawingView:unifiedView.stats,faceVisibility:unifiedView.visibility?.stats,projectedCadGraph:projectedGraph.stats,cleanup,bodyVisibility:bodyVisibilityStats(vb),exactCurveVisibility:exactCurveVisibilityStats(exactVisibility),analyticFaceHLR:analyticFaceHLRStats(analyticFaceHLR),trueSurfaceProjection:unifiedView.trueSurfaceProjection?.stats,selfOccludedSamples,foreignOccludedSamples}};
}
function dimH(x1,x2,y,y0,label){return`<g stroke="#111" fill="#111" stroke-width=".8" font-family="Arial,sans-serif" font-size="10"><line x1="${x1}" y1="${y0}" x2="${x1}" y2="${y+4}"/><line x1="${x2}" y1="${y0}" x2="${x2}" y2="${y+4}"/><line x1="${x1}" y1="${y}" x2="${x2}" y2="${y}"/><path d="M${x1} ${y}l6 -2.4v5.0zM${x2} ${y}l-6 -2.4v5.0z"/><text x="${(x1+x2)/2}" y="${y-4}" text-anchor="middle" stroke="none">${esc(label)}</text></g>`}
...
function renderSection(rec,s,box,planePoint,planeNormal,{stroke='#111',width=.8,hatch=true,frameBounds=null,includeComponents=null,sectionId=''}={}){
  ...
  for(const f of rec.faces||[]){if(includeComponents&& !includeComponents.has(f.componentId||'RAW'))continue;if(cylinderFaceKeys.has(fk(f)))continue;const seg=triPlaneSegment(f.loops?.[0],planePoint,planeNormal,Math.max(.012,q*1.4));if(!seg)continue;const k=sectionSegKey(seg[0],seg[1],f.componentId,q);if(seen.has(k))continue;seen.add(k);raw.push({a:seg[0],b:seg[1],componentId:f.componentId||'RAW'})}
  const chains=chainSectionSegments(raw,q),analyticCutRaw=analyticSectionCurves(rec,planePoint,planeNormal,{circleSegments:160,minConfidence:.78}),analyticCut={...analyticCutRaw,curves:includeComponents?analyticCutRaw.curves.filter(c=>includeComponents.has(c.componentId||'RAW')):analyticCutRaw.curves};
  ...
  const axialSource=includeComponents?{...analytic,cylinders:(analytic.cylinders||[]).filter(c=>includeComponents.has(c.componentId||'RAW'))}:analytic;
  const axialBodies=analyticAxialMaterialLoops(axialSource,planePoint,planeNormal,M);
  ...
  // v3.1: pair open cut chains belonging to thin plates/sheet parts into closed material strips.
  // This restores hatchable material for ribs, blades and welded sheet details whose tessellation
  // intersection is topologically open even though the physical section is a closed thin strip.
  const thinMaterial=reconstructSectionMaterial(rec,chains,M,{includeComponents});
  for(const [id,loops] of thinMaterial.loopsByComponent){let target=bodyLoops.get(id);if(!target){target=[];target.policy=sectionComponentPolicy(rec,id,planeNormal);bodyLoops.set(id,target)}for(const loop of loops){if(Math.abs(polygonArea2D(loop))>.9)target.push(loop)}}
  // v5.0: for closed reconstructed solids, use Shell-owned section contours as the
  // authoritative material topology. Nested shell loops are kept together so the hatch
  // even/odd rule naturally preserves bores/cavities. Open or uncertain components retain
  // the proven mesh/analytic/thin-sheet fallbacks above.
  // v5.0: replace raw closed Shell-section chains with canonical plane-local regions.
  // Exact Section Region Core repairs tiny closure defects, determines nested hole parity,
  // orients outer/hole loops consistently and returns deterministic per-component regions.
  const exactSection=reconstructExactSectionRegions(rec,planePoint,planeNormal,{includeComponents});
  const exactBoolean=reconstructExactSectionBoolean(rec,planePoint,planeNormal,{includeComponents});
  let solidSectionComponents=0,solidSectionLoops=0,exactSectionRegions=0,exactSectionHoles=0;
  for(const [id,entry] of exactSection.byComponent){const loops=[];for(const L of entry.loops||[]){const mapped=(L.points3D||[]).map(M.P);if(mapped.length<4)continue;if(Math.hypot(mapped[0][0]-mapped.at(-1)[0],mapped[0][1]-mapped.at(-1)[1])>.2)mapped.push(mapped[0]);if(Math.abs(polygonArea2D(mapped))>.9)loops.push(mapped)}if(!loops.length)continue;loops.policy=bodyLoops.get(id)?.policy||sectionComponentPolicy(rec,id,planeNormal);bodyLoops.set(id,loops);solidSectionComponents++;solidSectionLoops+=loops.length;exactSectionRegions+=(entry.regions||[]).length;exactSectionHoles+=(entry.loops||[]).filter(x=>x.role==='void-hole').length}
  // v5.0: assembly-level boolean boundary. Keep per-component loops for hatch direction,
  // but draw the actual union boundary and real component interfaces instead of every raw
  // tessellation cut edge. This removes doubled/overlapping cut contours at welded contacts.
  let booleanBoundaryD='',booleanInterfaceD='';
  for(const L of exactBoolean.loops||[]){const mapped=(L.points3D||[]).map(M.P);if(mapped.length<3)continue;booleanBoundaryD+=`M${mapped[0][0].toFixed(2)} ${mapped[0][1].toFixed(2)}`;for(let i=1;i<mapped.length;i++)booleanBoundaryD+=`L${mapped[i][0].toFixed(2)} ${mapped[i][1].toFixed(2)}`}
  for(const e of exactBoolean.interfaces||[]){const a=M.P(e.a3),b=M.P(e.b3);booleanInterfaceD+=`M${a[0].toFixed(2)} ${a[1].toFixed(2)}L${b[0].toFixed(2)} ${b[1].toFixed(2)}`}
  const finalBoundaryD=booleanBoundaryD||boundaryD;
  ...
  const svg=`<g data-section-core="v5.0" data-solid-region-core="v5.0" data-exact-section-region-core="v5.0" data-exact-section-boolean-core="v5.0" data-solid-section-components="${solidSectionComponents}" data-solid-section-loops="${solidSectionLoops}" data-exact-section-regions="${exactSectionRegions}" data-exact-section-holes="${exactSectionHoles}" data-exact-section-repaired="${exactSection.counts.repairedLoops}" data-boolean-loops="${exactBoolean.counts.booleanLoops}" data-boolean-holes="${exactBoolean.counts.holes}" data-boolean-interfaces="${exactBoolean.counts.interfaceSegments}" data-boolean-splits="${exactBoolean.counts.splitSegments}" data-section-chains="${chains.length}" data-section-bodies="${bodyLoops.size}" data-section-hatched-bodies="${hatchBundle.hatchedBodies}" data-section-unhatched-bodies="${hatchBundle.unhatchedBodies}" data-analytic-section-curves="${analyticCut.curves.length}" data-thin-material-components="${thinMaterial.components}" data-thin-material-pairs="${thinMaterial.paired}" data-welded-joints="${rec?.weldedAssembly?.counts?.joints||0}" data-welded-ribs="${rec?.weldedAssembly?.counts?.bladeRibs||0}" data-section-id="${esc(sectionId)}">${hatchBundle.svg}<path d="${finalBoundaryD}" fill="none" stroke="${stroke}" stroke-width="${Math.max(.62,width*.9)}" stroke-linecap="round" stroke-linejoin="round"/><path d="${booleanInterfaceD}" fill="none" stroke="${stroke}" stroke-width="${Math.max(.38,width*.55)}" stroke-linecap="round"/><path d="${analyticD}" fill="none" stroke="${stroke}" stroke-width="${Math.max(.58,width*.88)}" stroke-linecap="round" stroke-linejoin="round"/></g>`;
  return{svg,map:M,count:chains.length,bodies:bodyLoops.size,hatchedBodies:hatchBundle.hatchedBodies,unhatchedBodies:hatchBundle.unhatchedBodies,analyticCurves:analyticCut.curves.length,thinMaterialComponents:thinMaterial.components,thinMaterialPairs:thinMaterial.paired,solidSectionComponents,solidSectionLoops,exactSectionRegions,exactSectionHoles,exactSectionRepaired:exactSection.counts.repairedLoops,booleanLoops:exactBoolean.counts.booleanLoops,booleanHoles:exactBoolean.counts.holes,booleanInterfaces:exactBoolean.counts.interfaceSegments,analytic:true};
}
...
function viewLocalBounds(center,s,spanX,spanY,depth){
  const pts=[];for(const dx of [-spanX/2,spanX/2])for(const dy of [-spanY/2,spanY/2])for(const dz of [-depth/2,depth/2])pts.push(add(center,add(add(mul(s.px,dx),mul(s.py,dy)),mul(s.dir,dz))));
  const mn=[Infinity,Infinity,Infinity],mx=[-Infinity,-Infinity,-Infinity];for(const p of pts)for(let i=0;i<3;i++){mn[i]=Math.min(mn[i],p[i]);mx[i]=Math.max(mx[i],p[i])}return{min:mn,max:mx,size:sub(mx,mn),center:mul(add(mn,mx),.5)};
}
function spiralShellContact(rec,plan,station,side){
  const spiral=plan?.subfeatures?.spiral?.global,{axis,D}=plan;if(!spiral?.faces?.length)return pointOnAxis(plan.axisPoint,axis,station);
  const {u}=basis(axis),origin=plan.axisPoint,targetR=Math.max(plan.innerBore/2,plan.outerDiameter/2-plan.featureThickness*4),window=Math.max(80,D*.24);let best=null,bestScore=Infinity;
  for(const f of spiral.faces){for(const p of f.loops?.[0]||[]){const t=dot(p,axis),r=dot(sub(p,origin),u),radialErr=Math.abs(Math.abs(r)-targetR),axialErr=Math.abs(t-station);if(axialErr>window)continue;const score=axialErr*.72+radialErr*1.35;if(score<bestScore){bestScore=score;best=p}}}
  if(!best)return pointOnAxis(plan.axisPoint,axis,station);
  // Snap the detail center slightly toward the shell wall so both plate and shell remain in frame.
  const sign=dot(sub(best,origin),u)>=0?1:-1;return add(best,mul(u,sign*plan.featureThickness*1.2));
}
function detailWindowRecord(rec,center,s,spanX,spanY,depth){
  const hx=spanX*.62,hy=spanY*.62,hd=depth*.7,faces=(rec.faces||[]).filter(f=>{const c=faceCentroid(f),d=sub(c,center);return Math.abs(dot(d,s.px))<=hx&&Math.abs(dot(d,s.py))<=hy&&Math.abs(dot(d,s.dir))<=hd});return faces.length?subRecord(rec,faces):rec;
}
function renderCrossFeatureGraph(part,graph,s,box){
  const M=mapView(part.bounds,s,box,12),center=M.P(graph.hub.axisPoint),scale=M.scale,outerR=graph.hubDiameter*scale/2,boreR=(graph.boreDiameter||0)*scale/2;let bodyD='',holeD='';
  const circle=(c,r)=>`<circle cx="${c[0].toFixed(2)}" cy="${c[1].toFixed(2)}" r="${Math.max(.3,r).toFixed(2)}"/>`;
...
function renderDrumReferenceSheet(svg,rec,plan,{projectName,fileName,theme,mode='assemblyDetailed'}){
  ...
  const sectionPlan=planAssemblySections(rec,plan);rec.sectionPlan=sectionPlan;rec.weldSeams=rec.weldSeams||reconstructWeldSeams(rec);
  const aRadial=norm(cross(sectionPlan.A.n,axis)),aSectionView={px:axis,py:aRadial,dir:sectionPlan.A.n};
  const composition=composeDrumA2(rec,plan,{mode,sectionPlan,hasCross:!!subfeatures.cross});rec.drawingComposition=composition;
  const {top:topBox,end:endBox,cross:crossBox,main:mainBox,isoShell:isoShellBox,aa:aaBox,bb:bbBox,detailD:detailDBox,isoOpen:isoOpenBox,bom:bomBox,stamp:stampBox}=composition.boxes;
  const annotationLayout=createAnnotationLayout([bomBox,stampBox,isoShellBox,isoOpenBox].map((b,i)=>({id:['bom','stamp','iso-shell','iso-open'][i],...b})));rec.annotationLayout=annotationLayout;
  const dimensionPlan=planDrumDimensions(rec,plan,composition);rec.dimensionPlan=dimensionPlan;
  const functionalPlan=buildFunctionalDimensionPlan(rec,plan,composition,dimensionPlan,{crossGraph:subfeatures.cross?.graph||null});rec.functionalDimensionPlan=functionalPlan;
  const leaderRouter=createProductionLeaderRouter(annotationLayout,{sheet:composition.sheet.frame,fontSize:9});rec.productionLeaders=leaderRouter.leaders;
  const draftingGraph=buildProductionDraftingGraph(rec,plan,composition,sectionPlan,functionalPlan);rec.productionDraftingGraph=draftingGraph;
  const viewSynthesis=buildProductionViewSynthesis(rec,plan,composition,{crossGraph:subfeatures.cross?.graph||null});rec.productionViewSynthesis=viewSynthesis.stats;
  ...
  let s=`<g data-drawing-profile="DRUM_REFERENCE_A2" data-drawing-mode="${mode}" data-source-scope="assembly-descendants" data-view-planner="engineering-composer+functional-dimensions+production-leaders+section-context+view-topology+production-view-synthesizer-v14.0" data-composer="v14.0" data-semantic-dimension-planner="v14.0" data-functional-dimension-core="v14.0" data-production-leader-core="v14.0" data-fidelity-guard="v14.0">${referenceA2Frame(theme)}<metadata id="rozfood-composer">${esc(JSON.stringify(composerDiagnostics(composition)))}</metadata>`;
  const top=viewSynthesis.top,front=viewSynthesis.end,main=viewSynthesis.main;
  const productionHelix={svg:'',stats:{surfaces:0,curves:viewSynthesis.stats.helicoidCurves,pitch:rec.parametricHelicoids?.stats?.meanPitch||null}};
  const aaCut=renderSection(rec,aSectionView,aaBox,sectionPlan.A.point,sectionPlan.A.n,{width:.72,hatch:true,includeComponents:sectionPlan.A.includeComponents,sectionId:'A-A'});
  const aaContextRec=sectionContextRecord(rec,sectionPlan.A.point,sectionPlan.A.n,{depth:Math.max(48,D*.18),includeComponents:sectionPlan.A.includeComponents});
  const aaContext=renderMesh(aaContextRec,aSectionView,aaBox,{width:.36,detail:true,hiddenRemoval:true,frameBounds:rec.bounds,fixedScale:aaCut.map.scale});
  rec.sectionContext=aaContextRec.sectionContext;
  const productionAA=renderProductionAAOverlay(rec,plan,aaCut.map,aaBox);
  const dStation=sectionPlan.D.station,dPoint=spiralShellContact(rec,plan,dStation,aSectionView),dBounds=viewLocalBounds(dPoint,aSectionView,Math.max(62,D*.15),Math.max(55,D*.14),Math.max(28,D*.08)),dRec=detailWindowRecord(rec,dPoint,aSectionView,Math.max(62,D*.15),Math.max(55,D*.14),Math.max(28,D*.08)),dCut=renderSection(dRec,aSectionView,detailDBox,dPoint,sectionPlan.A.n,{width:.82,hatch:true,frameBounds:dBounds,sectionId:'D'}),dView={svg:dCut.svg,map:dCut.map};
  const bbPoint=sectionPlan.B.point,bbCut=viewSynthesis.bb;
  ...
  const mainWeld=renderWeldSeams(rec,main.map,{box:mainBox,showPaths:true,showCallout:cfg.localDimensions,maxCallouts:1});
  s+=viewLabel('A',mainBox.x+4,mainBox.y-10)+clipView(main.svg+productionHelix.svg+mainWeld.svg,mainBox,'clipMain');
  if(cfg.sections)s+=`<g data-section-angle="${sectionPlan.A.deg}" data-section-components="${sectionPlan.A.includeComponents.size}" data-section-context="v14.0">${viewLabel('A–A',aaBox.x+4,aaBox.y-12,'1:7')}${clipView(`<g opacity=".22" data-section-visible-context="true">${aaContext.svg}</g>${aaCut.svg}${productionAA.svg}`,aaBox,'clipAA')}</g>`;
  if(cfg.details)s+=`<g data-section-station="${sectionPlan.B.station.toFixed(3)}" data-section-components="${sectionPlan.B.includeComponents.size}">${viewLabel('B–B',bbBox.x+4,bbBox.y-12,'1:5')}${clipView(bbCut.svg,bbBox,'clipBB','circle')}</g>`+`<circle cx="${bbBox.x+bbBox.w/2}" cy="${bbBox.y+bbBox.h/2}" r="${Math.min(bbBox.w,bbBox.h)/2-3}" fill="none" stroke="#aaa" stroke-width=".6"/>`;
  const dWeld=renderLocalWeldDetail(rec,dView.map,detailDBox,{label:'Сварной шов'}),productionD=renderProductionDetailD(plan,detailDBox);
  if(cfg.details)s+=viewLabel('D',detailDBox.x+4,detailDBox.y-12,'2:5')+clipView(productionD.svg,detailDBox,'clipD','circle')+`<circle cx="${detailDBox.x+detailDBox.w/2}" cy="${detailDBox.y+detailDBox.h/2}" r="${Math.min(detailDBox.w,detailDBox.h)/2-3}" fill="none" stroke="#888" stroke-width=".7"/>`;
  ...
  if(cfg.cross&&subfeatures.cross){const crossPlan=viewSpec(subfeatures.cross.graph.hubAxis,'end'),graph=subfeatures.cross.graph,crossView=renderCrossFeatureGraph(subfeatures.cross.part,graph,crossPlan,crossBox),pc=crossView.map.P(graph.hub.axisPoint),frame=basis(graph.hubAxis),ha=crossView.map.P(add(graph.hub.axisPoint,mul(frame.u,graph.hubDiameter/2))),hb=crossView.map.P(add(graph.hub.axisPoint,mul(frame.u,-graph.hubDiameter/2)));s+=viewLabel('C',crossBox.x+4,crossBox.y-9,'2:3')+clipView(crossView.svg,crossBox,'clipCross')+dimV(crossBox.x+crossBox.w-10,Math.min(ha[1],hb[1]),Math.max(ha[1],hb[1]),Math.max(ha[0],hb[0]),`Ø ${fmt(graph.hubDiameter,0)}`);const holeText=`${graph.holeCount} отв. Ø${fmt(graph.holeDiameter,0)} · PCD Ø${fmt(graph.holePcd,0)}`;const leader=leaderRouter.route([pc[0]+Math.max(12,graph.holePcd*crossView.map.scale*.48),pc[1]],holeText,{preferred:'top',id:'mounting-pattern',viewBox:crossBox,priority:8});s+=leaderRouter.render(leader)}
  ...
  const endCenter=front.map.P(rec.bounds.center);if(cfg.dimensions)dimensionPlan.end.forEach((dim,index)=>{const diameter=dim.value,half=diameter*front.map.scale/2;s+=dimH(endCenter[0]-half,endCenter[0]+half,dim.lane,endCenter[1],`Ø ${fmt(diameter,0)}`)});
  ...
  const mc=main.map.P(rec.bounds.center);if(cfg.dimensions)dimensionPlan.shell.slice(0,3).forEach((dim,index)=>{const diameter=dim.value,half=diameter*main.map.scale/2;s+=dimV(dim.lane,mc[1]-half,mc[1]+half,mainBox.x+7,`Ø ${fmt(diameter,0)}`)});
  const baseSmall=plan.smallSegments.filter(segment=>segment.length>=10&&segment.length<=165),usefulSmall=[];if(baseSmall.length>1&&Math.abs(baseSmall[0].b-baseSmall[1].a)<.8&&baseSmall[0].length+baseSmall[1].length<65)usefulSmall.push({a:baseSmall[0].a,b:baseSmall[1].b,length:baseSmall[0].length+baseSmall[1].length});usefulSmall.push(...baseSmall.filter((_,index)=>index!==1).slice(0,4));if(cfg.localDimensions)usefulSmall.forEach((segment,index)=>{const xa=main.map.P(pointOnAxis(plan.axisPoint,axis,segment.a))[0],xb=main.map.P(pointOnAxis(plan.axisPoint,axis,segment.b))[0];s+=dimH(xa,xb,composition.lanes.mainBottom[index%composition.lanes.mainBottom.length],mainBox.y+mainBox.h-5,fmt(segment.length,0))});
  if(cfg.localDimensions&&plan.spiralPitch){const pitch=Math.round(plan.spiralPitch/10)*10,total=Math.round(Math.max(0,plan.spiralSpan-plan.featureThickness));s+=`<g font-family="Arial" fill="#111" stroke="#111" stroke-width=".65"><text x="${mainBox.x+470}" y="${mainBox.y+mainBox.h+68}" stroke="none" font-size="10">${fmt(pitch,0)}        ${fmt(pitch,0)}        ${fmt(total,0)}</text></g>`}
  if(cfg.localDimensions){const aaAnchor=aaCut.map.P(add(sectionPlan.A.point,mul(aRadial,Math.max(8,D*.08))));const tDim=functionalPlan.manufacturing.find(x=>x.role==='plate-thickness');if(tDim){const Ld=leaderRouter.route(aaAnchor,`t = ${fmt(tDim.value,1)} мм`,{preferred:'right',id:'section-thickness',viewBox:aaBox,priority:6});s+=leaderRouter.render(Ld)}const pitchDim=functionalPlan.manufacturing.find(x=>x.role==='helical-pitch');if(pitchDim){const pAnchor=main.map.P(pointOnAxis(plan.axisPoint,axis,(plan.body?.min||plan.min)+Math.min(plan.body?.length||L,L)*.62));const Lp=leaderRouter.route([pAnchor[0],pAnchor[1]-Math.max(10,D*main.map.scale*.18)],`Шаг ${fmt(pitchDim.value,0)} мм`,{preferred:'bottom',id:'helical-pitch',viewBox:mainBox,priority:5});s+=leaderRouter.render(Lp)}}
  ...
  s+=`<text x="102" y="1152" font-family="Arial" font-size="7.5" fill="#555">ROZFOOD ENGINEERING STUDIO · Drawing Intelligence v14.0.1 · PRODUCTION VIEW SYNTHESIZER · ${cfg.label} · ${esc(fileName)}</text><text x="1628" y="1152" text-anchor="end" font-family="Arial" font-size="7.5">Формат А2</text></g>`;
  svg.innerHTML=s;
}
...
function precisionNotes(rec,limit=5){
  const out=[];for(const g of rec?.recognition?.coaxialGroups||[]){if(g.diameters?.length<2)continue;out.push(`Соосные Ø ${g.diameters.slice(0,4).map(d=>fmt(d,2)).join(' / ')}`);if(out.length>=limit)return out}for(const note of featureDrawingNotes(rec,{limit})){if(!out.includes(note))out.push(note);if(out.length>=limit)break}return out;
}
function renderGeneralAssemblySheet(svg,rec,{projectName='SLDASM',fileName='',theme='light',mode='assemblyDetailed'}={}){
  ...
  s+=`<g font-family="Arial" fill="#111"><text x="${large?105:95}" y="${H-42}" font-size="7" fill="#555">ROZFOOD ENGINEERING STUDIO · Drawing Intelligence v14.0.1 · GENERAL · ${cfg.label} · ${large?'A1':'A2'} · ${esc(fileName)}</text><text x="${W-145}" y="${bomBox.y-12}" font-size="9" text-anchor="end">VERIFIED TESS</text></g>`;
  svg.innerHTML=s;
}
...
export function renderAssemblyProductionSheet(svg,rec,{projectName='SLDASM',fileName='',theme='light',mode='assemblyDetailed'}={}){
  ...
  rec.manufacturing=rec.manufacturing||recognizeManufacturingFeatures(rec);
  rec.cadFeatures=rec.cadFeatures||reconstructCadFeatureEntities(rec);
  rec.weldedAssembly=rec.weldedAssembly||recognizeWeldedAssembly(rec);
  rec.solidRegions=rec.solidRegions||reconstructSolidRegions(rec);
  ...
  const sectionPlan=planAssemblySections(rec,drumPlan);rec.sectionPlan=sectionPlan;
  ...
  const cut=renderSection(rec,side,mainSection,sectionPlan.A.point,sectionPlan.A.n,{width:.75,hatch:true,includeComponents:sectionPlan.A.includeComponents,sectionId:'A-A'});
  const hub=crossHubDetail(rec,axis,D),cView=renderMesh(hub,end,detailCBox,{width:.8,detail:true});
  const dRec=detailAxial(rec,axis,sectionPlan.D.station,D),dView=renderMesh(dRec,side,detailDBox,{width:.62,detail:true});
  const aaCut=renderSection(rec,side,sectionAA,sectionPlan.A.point,sectionPlan.A.n,{width:.72,hatch:true,includeComponents:sectionPlan.A.includeComponents,sectionId:'A-A'}),aa={svg:aaCut.svg,map:aaCut.map};
  const iso1=renderMesh(rec,iso,isoSolid,{width:.5});
  const bbCut=renderSection(rec,end,isoExpl,sectionPlan.B.point,sectionPlan.B.n,{width:.74,hatch:true,includeComponents:sectionPlan.B.includeComponents,sectionId:'B-B'}),iso2={svg:bbCut.svg,map:bbCut.map};
  ...
  s+=`<g font-family="Arial" fill="#111"><text x="95" y="944" font-size="7" fill="#555">ROZFOOD ENGINEERING STUDIO · Drawing Intelligence v14.0.1 · ${esc(fileName)}</text><text x="1240" y="612" font-size="9" text-anchor="end">VERIFIED TESS</text></g>`;
  svg.innerHTML=s;
}
...
function renderGeneralComponentSheet(svg,part,{componentName='Деталь',fileName='',theme='light'}={}){
  ...
  s+=`<text x="95" y="944" font-family="Arial" font-size="7" fill="#555">ROZFOOD ENGINEERING STUDIO · Drawing Intelligence v14.0.1 · GENERAL PART · ${esc(fileName)}</text><text x="1240" y="815" font-family="Arial" font-size="9" text-anchor="end" fill="#111">VERIFIED TESS</text>`;
  svg.innerHTML=s;
}
...
export function renderComponentProductionSheet(svg,rec,{componentId=null,componentName='Деталь',fileName='',theme='light'}={}){
  ...
  part.recognition=part.recognition||recognizeTessellationGeometry(part);part.manufacturing=recognizeManufacturingFeatures(part);part.cadFeatures=reconstructCadFeatureEntities(part);
  ...
  s+=renderPartStamp(componentName,scale,{x:860,y:845,w:495,h:95});
  s+=`<text x="95" y="944" font-family="Arial" font-size="7" fill="#555">ROZFOOD ENGINEERING STUDIO · Drawing Intelligence v14.0.1 · ${esc(fileName)}</text><text x="1240" y="815" font-family="Arial" font-size="9" text-anchor="end" fill="#111">VERIFIED TESS</text>`;
  svg.innerHTML=s;
}
