...
function renderMesh(rec,s,box,{stroke='#111',width=.72,detail=false,fixedScale=null,hiddenRemoval=true,frameBounds=null}={}){
  ...
  return{svg:`<g data-reconstruction="v2.6" data-analytic-cylinders="${analytic.counts.cylinders}" data-cad-boundaries="${analytic.counts.cadBoundaries||0}" data-suppressed-analytic-mesh="${suppressedAnalyticMesh}"><path d="${featureD}" fill="none" stroke="${stroke}" stroke-width="${featureWidth}" stroke-linecap="round" stroke-linejoin="round"/><path d="${analyticD}" fill="none" stroke="${stroke}" stroke-width="${analyticWidth}" stroke-linecap="round" stroke-linejoin="round"/><path d="${contourD}" fill="none" stroke="${stroke}" stroke-width="${contourWidth}" stroke-linecap="round" stroke-linejoin="round"/></g>`,map:M,hiddenRemoval:!!vb,reconstruction:{...stats,analyticCylinders:analytic.counts.cylinders,cadBoundaries:analytic.counts.cadBoundaries||0,suppressedAnalyticMesh,analyticSegments,visibleSegments}};
}
...
function renderDrumReferenceSheet(svg,rec,plan,{projectName,fileName,theme,mode='assemblyDetailed'}){
  ...
  const dStation=chain.stations[Math.max(0,Math.min(chain.stations.length-1,Math.floor(chain.stations.length*.62)))]??dot(rec.bounds.center,axis),dPoint=spiralShellContact(rec,plan,dStation,side),dBounds=viewLocalBounds(dPoint,side,Math.max(62,D*.15),Math.max(55,D*.14),Math.max(28,D*.08)),dRec=detailWindowRecord(rec,dPoint,side,Math.max(62,D*.15),Math.max(55,D*.14),Math.max(28,D*.08)),dCut=renderSection(dRec,side,detailDBox,dPoint,v,{width:.82,hatch:true,frameBounds:dBounds}),dView={svg:dCut.svg,map:dCut.map};
...
function renderGeneralAssemblySheet(svg,rec,{projectName='SLDASM',fileName='',theme='light',mode='assemblyDetailed'}={}){
  ...
  s+=`<g font-family="Arial" fill="#111"><text x="${large?105:95}" y="${H-42}" font-size="7" fill="#555">ROZFOOD ENGINEERING STUDIO · Drawing Intelligence v2.6.0 · GENERAL · ${cfg.label} · ${large?'A1':'A2'} · ${esc(fileName)}</text><text x="${W-145}" y="${bomBox.y-12}" font-size="9" text-anchor="end">VERIFIED TESS</text></g>`;
  svg.innerHTML=s;
}
...
export function renderAssemblyProductionSheet(svg,rec,{projectName='SLDASM',fileName='',theme='light',mode='assemblyDetailed'}={}){
  ...
  s+=`<g font-family="Arial" fill="#111"><text x="95" y="944" font-size="7" fill="#555">ROZFOOD ENGINEERING STUDIO · Drawing Intelligence v2.6.0 · ${esc(fileName)}</text><text x="1240" y="612" font-size="9" text-anchor="end">VERIFIED TESS</text></g>`;
  svg.innerHTML=s;
}
...
function renderGeneralComponentSheet(svg,part,{componentName='Деталь',fileName='',theme='light'}={}){
  ...
  s+=`<text x="95" y="944" font-family="Arial" font-size="7" fill="#555">ROZFOOD ENGINEERING STUDIO · Drawing Intelligence v2.6.0 · GENERAL PART · ${esc(fileName)}</text><text x="1240" y="815" font-family="Arial" font-size="9" text-anchor="end" fill="#111">VERIFIED TESS</text>`;
  svg.innerHTML=s;
}
...
export function renderComponentProductionSheet(svg,rec,{componentId=null,componentName='Деталь',fileName='',theme='light'}={}){
  ...
  s+=renderPartStamp(componentName,scale,{x:860,y:845,w:495,h:95});
  s+=`<text x="95" y="944" font-family="Arial" font-size="7" fill="#555">ROZFOOD ENGINEERING STUDIO · Drawing Intelligence v2.6.0 · ${esc(fileName)}</text><text x="1240" y="815" font-family="Arial" font-size="9" text-anchor="end" fill="#111">VERIFIED TESS</text>`;
  svg.innerHTML=s;
}
