...
function componentQuantity(rec,componentId,fileName){const components=rec?.nativeAssembly?.components||rec?.components||[];const direct=components.find(item=>(item.instances||[]).includes(componentId));if(direct)return direct.count||1;const key=String(fileName||'').toLowerCase();return components.find(item=>String(item.file||'').toLowerCase()===key)?.count||1}
...
export function renderComponentProductionSheet(svg,rec,{componentId=null,componentName='Деталь',fileName='',theme='light'}={}){
  ...
  const featureGraph=part.featureGraph=part.featureGraph||buildFeatureGraph(part),quantity=componentQuantity(rec,componentId,fileName);
