...
export function renderDrawing(svg,drawing,options={}){
  ...
  s+=`<text x="55" y="715" fill="${colors.muted}" font-family="system-ui,-apple-system,sans-serif" font-size="11">ROZFOOD ENGINEERING STUDIO · Drawing Intelligence v1.5.0 · SLDASM FaceTessellations локально</text>`;
  svg.innerHTML=s;
}
...
function renderTitleBlock(meta,box,c){
  ...
  s+=row('ЯДРО','v1.5.0',y+99,{mono:true,size:8.8});
  s+='</g>';
  return s;
}
...
export function renderNativeAssemblyDrawing(svg,nativeAssembly,options={}){
  ...
  s+=`<text x="55" y="665" fill="${c.muted}" font-family="system-ui" font-size="11">ROZFOOD ENGINEERING STUDIO · Drawing Intelligence v1.5.0 · SLDASM Tess Recognition</text>`;
