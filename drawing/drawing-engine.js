...
export function renderDrawing(svg,drawing,options={}){
  ...
  s+=`<text x="55" y="715" fill="${colors.muted}" font-family="system-ui,-apple-system,sans-serif" font-size="11">ROZFOOD ENGINEERING STUDIO · Drawing Intelligence v2.6 · CAD Edge Graph + Analytic HLR · SLDASM локально</text>`;
  svg.innerHTML=s;
}
...
function renderTitleBlock(meta,box,c){
  ...
  s+=row('ЯДРО','v2.6.0',y+99,{mono:true,size:8.8});
  s+='</g>';
  return s;
}
...
export function renderNativeAssemblyDrawing(svg,nativeAssembly,options={}){
  ...
  s+=`<text x="55" y="665" fill="${c.muted}" font-family="system-ui" font-size="11">ROZFOOD ENGINEERING STUDIO · Drawing Intelligence v2.6.0 · Analytic/TESS Recognition</text>`;
