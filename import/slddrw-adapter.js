...
export async function parseSLDDRW(buffer,{fileName='drawing.slddrw'}={}){
  ...
  if(container==='ole2') throw new Error('Этот SLDDRW использует старый OLE2-контейнер (до SOLIDWORKS 2015). В v2.0.0 он пока только определяется, но не читается.');
  if(container==='opc') throw new Error('Этот файл использует OPC/ZIP-контейнер. В v2.0.0 поддержан modern SLDDRW.');
