const CACHE='rozfood-engineering-studio-v14.0.2-production-view-synthesizer';
const ASSETS=[
  './','./index.html','./styles.css?v=14.0.2','./app.js?v=14.0.2','./import-worker.js?v=14.0.2',
...
self.addEventListener('fetch',e=>{
  ...
  e.respondWith(fetch(req,{cache:'no-store'}).then(resp=>{if(resp&&resp.ok){const copy=resp.clone();caches.open(CACHE).then(c=>c.put(req,copy));}return resp;}).catch(()=>caches.match(req).then(r=>r||caches.match(req.url.replace(self.location.origin,'')))));
});
