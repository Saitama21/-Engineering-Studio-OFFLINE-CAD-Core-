...
export class WireframeViewer{
  ...
  drawFaces(c,scale,cx,cy,center){
    ...
    for(const item of faces){const {f,loops}=item,selected=!this.selectedComponent||f.componentId===this.selectedComponent,alpha=this.selectedComponent?(selected?.98:.05):.94;c.beginPath();for(const loop of loops){c.moveTo(loop[0][0],loop[0][1]);for(let i=1;i<loop.length;i++)c.lineTo(loop[i][0],loop[i][1]);c.closePath()}
