...
export class WireframeViewer{
  constructor(canvas){
    this.canvas=canvas;this.ctx=canvas.getContext('2d');this.rec=null;this.rx=-.45;this.ry=.7;this.zoom=1;this.pan=[0,0];this.drag=null;this.down=null;this.moved=0;this.mode='solid';this.selectedComponent=null;this.selectedIds=new Set();this.selectedBounds=null;this.hitFaces=[];this.onSelect=null;this.framePending=false;this.interacting=false;this.interactionTimer=null;this.sectionAxis=null;this.sectionFraction=.5;
  ...
  setSectionAxis(axis=null,fraction=.5){this.sectionAxis=[0,1,2].includes(axis)?axis:null;this.sectionFraction=Math.max(.05,Math.min(.95,Number(fraction)||.5));this.draw()}
  getSectionState(){return{axis:this.sectionAxis,fraction:this.sectionFraction}}
  ...
  sectionValue(){const b=this.selectedBounds||this.rec?.bounds;if(this.sectionAxis==null||!b)return null;return b.min[this.sectionAxis]+b.size[this.sectionAxis]*this.sectionFraction}
  clipLoop(loop){const axis=this.sectionAxis,cut=this.sectionValue();if(axis==null||cut==null)return loop;let input=[...loop],out=[];for(let i=0;i<input.length;i++){const a=input[i],b=input[(i+1)%input.length],ina=a[axis]<=cut,inb=b[axis]<=cut;if(ina)out.push(a);if(ina!==inb){const t=(cut-a[axis])/((b[axis]-a[axis])||1e-12);out.push([a[0]+(b[0]-a[0])*t,a[1]+(b[1]-a[1])*t,a[2]+(b[2]-a[2])*t])}}return out}
  clipSegment(a,b){const axis=this.sectionAxis,cut=this.sectionValue();if(axis==null||cut==null)return[a,b];const ina=a[axis]<=cut,inb=b[axis]<=cut;if(!ina&&!inb)return null;if(ina&&inb)return[a,b];const t=(cut-a[axis])/((b[axis]-a[axis])||1e-12),q=[a[0]+(b[0]-a[0])*t,a[1]+(b[1]-a[1])*t,a[2]+(b[2]-a[2])*t];return ina?[a,q]:[q,b]}
  drawFaces(c,scale,cx,cy,center){
    ...
    for(let fi=0;fi<source.length;fi+=step){const f=source[fi];const loops=(f.loops||[]).map(loop=>this.clipLoop(loop)).filter(loop=>loop.length>2).map(loop=>loop.map(p=>this.project(p,scale,cx,cy,center))).filter(l=>l.length>2);if(!loops.length)continue;const all=loops.flat(),depth=all.reduce((s,p)=>s+p[2],0)/all.length;faces.push({f,loops,depth})}
  ...
  draw(){
    ...
    const segs=[],edgeSource=this.rec.edges||[],edgeStep=this.interacting&&edgeSource.length>42000?Math.ceil(edgeSource.length/42000):1;for(let ei=0;ei<edgeSource.length;ei+=edgeStep){const e=edgeSource[ei];const pts=e.points?.length?e.points:(e.p1&&e.p2?[e.p1,e.p2]:[]);for(let i=0;i<pts.length-1;i++){const clipped=this.clipSegment(pts[i],pts[i+1]);if(!clipped)continue;const a=this.project(clipped[0],scale,cx,cy,center),bb=this.project(clipped[1],scale,cx,cy,center);segs.push([a,bb,(a[2]+bb[2])/2,e])}}
    ...
    c.globalAlpha=1;c.fillStyle=muted;c.font='12px ui-monospace,SFMono-Regular,Menlo,monospace';const sceneEdges=this.rec.edges?.length??this.rec.counts.sceneEdges??0,sceneFaces=this.rec.counts.sceneFaces??this.rec.faces?.length??0,parts=this.rec.counts.sceneComponents??this.rec.components?.length??0;const bc=this.rec.brep?.counts,brepText=this.mode==='brep'&&bc?` · B-Rep ${bc.vertices}V / ${bc.edges}E / ${bc.loops||0}L / ${bc.faces}F / ${bc.shells} shells${this.rec.brep.topologyComplete&&Number.isFinite(bc.closedShells)?` / ${bc.closedShells} closed`:''}`:'';const sectionText=this.sectionAxis!=null?` · SECTION ${['X','Y','Z'][this.sectionAxis]} @ ${Math.round(this.sectionFraction*100)}%`:'';c.fillText(`${parts?parts+' components · ':''}${sceneFaces} triangles · ${sceneEdges} display edges${brepText}${sectionText}`,14,h-16);
