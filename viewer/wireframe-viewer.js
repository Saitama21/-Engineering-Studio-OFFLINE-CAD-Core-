...
export class WireframeViewer{
  constructor(canvas){
    this.canvas=canvas;this.ctx=canvas.getContext('2d');this.rec=null;this.rx=-.45;this.ry=.7;this.zoom=1;this.pan=[0,0];this.drag=null;this.down=null;this.moved=0;this.mode='solid';this.selectedComponent=null;this.hitFaces=[];this.onSelect=null;
  ...
  bind(){
    ...
    this.canvas.addEventListener('pointermove',e=>{if(!this.drag)return;const dx=e.clientX-this.drag[0],dy=e.clientY-this.drag[1];this.moved+=Math.abs(dx)+Math.abs(dy);this.ry+=dx*.008;this.rx+=dy*.008;this.drag=[e.clientX,e.clientY];this.draw()});
    ...
    this.canvas.addEventListener('wheel',e=>{e.preventDefault();this.zoom*=Math.exp(-e.deltaY*.001);this.zoom=Math.max(.12,Math.min(10,this.zoom));this.draw()},{passive:false});
  }
  hitTest(x,y){for(let i=this.hitFaces.length-1;i>=0;i--){const h=this.hitFaces[i];if(h.poly?.length>2&&pointInPoly([x,y],h.poly)){this.selectedComponent=this.selectedComponent===h.componentId?null:h.componentId;this.draw();if(this.onSelect)this.onSelect(this.selectedComponent,h.instance);return}}this.selectedComponent=null;this.draw();if(this.onSelect)this.onSelect(null,null)}
  ...
  componentStyle(id,alpha=.45){const h=hashString(id||'model')%36;const hue=206+(h-18)*.9;const dark=document.documentElement.dataset.theme==='dark';return`hsla(${hue} ${dark?45:48}% ${dark?42:67}% / ${alpha})`}
  drawFaces(c,scale,cx,cy,center){
    ...
    for(const item of faces){const {f,loops}=item,selected=!this.selectedComponent||f.componentId===this.selectedComponent,alpha=this.selectedComponent?(selected?.72:.06):.42;c.beginPath();for(const loop of loops){c.moveTo(loop[0][0],loop[0][1]);for(let i=1;i<loop.length;i++)c.lineTo(loop[i][0],loop[i][1]);c.closePath()}
      ...
      if(f.componentId)c.fillStyle=this.componentStyle(f.componentId,alpha*shade);else c.fillStyle=dark?`rgba(70,112,158,${alpha*.7})`:`rgba(153,186,219,${alpha})`;try{c.fill('evenodd')}catch{c.fill()}
  ...
  draw(){
    ...
    segs.sort((a,b)=>a[2]-b[2]);for(const [a,bb,,e] of segs){const selected=!this.selectedComponent||e.componentId===this.selectedComponent;c.globalAlpha=this.selectedComponent?(selected?1:.08):(this.mode==='solid'?.68:.88);c.strokeStyle=selected&&this.selectedComponent?'#006cff':(e.kind==='circle'?circle:wire);c.lineWidth=selected&&this.selectedComponent?2.2:(this.mode==='solid'?.85:(e.kind==='circle'?1.5:1.05));c.beginPath();c.moveTo(a[0],a[1]);c.lineTo(bb[0],bb[1]);c.stroke()}
    c.globalAlpha=1;c.fillStyle=muted;c.font='12px ui-monospace,SFMono-Regular,Menlo,monospace';const sceneEdges=this.rec.counts.sceneEdges??this.rec.edges?.length??0,sceneFaces=this.rec.counts.sceneFaces??this.rec.faces?.length??0,parts=this.rec.counts.sceneComponents??this.rec.components?.length??0;c.fillText(`${parts?parts+' components · ':''}${sceneFaces} faces · ${sceneEdges} edges`,14,h-16);
    if(this.selectedComponent){const comp=(this.rec.components||[]).find(x=>x.id===this.selectedComponent);if(comp){c.fillStyle=themeColor('--text-strong','#17202b');c.font='600 13px -apple-system,BlinkMacSystemFont,system-ui';c.fillText(`Выбрано: ${comp.name}`,14,24)}}
  }
}
